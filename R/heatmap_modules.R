...
generate_heatmap_UI <- function(id) {
    ...
    shiny::tagList(
        ...
        shiny::conditionalPanel(
            ...
            shiny::textInput(ns("legend_title"),
                      "Add a label to the legend")
        )
    )
}
...
generate_heatmap_server <- function(id, dat, filters) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        shiny::observeEvent(dat(), {
            ...
            shiny::updateSelectInput(
                ...
                choices = c("Number of interactions",
                            names(which(purrr::map_lgl(dat(), is.numeric))))
            )
        })
        ...
        heatmap_plot <- shiny::reactive({
            ...
            shiny::req(input$select_column %in% colnames(dat()))
            shiny::req(input$select_row %in% colnames(dat()))
...
download_heatmap_server <- function(id, heatmap_plot) {
    shiny::moduleServer(id, function(input, output, session) {
        shiny::observeEvent(heatmap_plot(), {
...
download_table_UI <- function(id) {
    ...
    shiny::tagList(
        ...
        shiny::textOutput(ns("preview_text")),
...
download_table_server <- function(id, heatmap_plot) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        col_names <- shiny::reactive({colnames(heatmap_plot()$data)})

        # Convert the table to wide format
        wide_table <- shiny::reactive({
