...
select_data_UI <- function(id) {
    ...
    shiny::selectInput(
        ns("select"),
        "Select a dataset",
        choices = c("Human APID (old)", "COFpendium")
    )
}
...
filter_data_UI <- function(id) {
    ...
    shiny::tagList(
        ...
        shiny::actionButton(ns("add_filter"), "Add a filter"),

        # Button to remove filters
        shiny::actionButton(ns("remove_filter"), "Remove a filter")
    )
}
...
filter_data_server <- function(id, dat) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        num_filters <- shiny::reactiveVal(0)
        max_num_filters <- shiny::reactiveVal(0)

        # Disable the remove filter button if there are no filters
        shiny::observe({
            if (num_filters() == 0) {
                shinyjs::disable("remove_filter")
            } else {
                shinyjs::enable("remove_filter")
            }
        })
        ...
        remove_filter <- function() {
            ...
            # Remove filter div
            shiny::removeUI(paste('#filter_div', num_filters(), sep = "_"))
...
display_data_UI <- function(id) {
    ...
    DT::dataTableOutput(ns("display_dataset"))
}
